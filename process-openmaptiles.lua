...
landuseKeys     = Set { "school", "university", "kindergarten", "college", "library", "hospital",
                        "railway", "aerialway", "cemetery", "military", "residential", "commercial", "industrial",
...
poiClassRanks   = { hospital=1, railway=2, bus=3, attraction=4, harbor=5, college=6,
					...
					fast_food=20, clothing_store=21, bar=22, aerialway=23 }
poiKeys         = Set { "amenity", "sport", "tourism", "office", "historic", "leisure", "landuse", "information", "railway", "aerialway" }
...
function way_function(way)
	...
	if highway~="" then
		...
		-- Set 'sac_scale' and associated
		-- "hiking", "mountain_hiking", "alpine_hiking", "demanding_alpine_hiking", "demanding_alpine_hiking"
		way:Layer(layer, false)
		way:Attribute("class", h)
		local sac_scale = way:Find("sac_scale")
		if sac_scale == "hiking" then way:Attribute("sac_scale", sac_scale) end
		if sac_scale == "mountain_hiking" then way:Attribute("sac_scale", sac_scale) end
		if sac_scale == "demanding_mountain_hiking" then way:Attribute("sac_scale", sac_scale) end
		if sac_scale == "alpine_hiking" then way:Attribute("sac_scale", sac_scale) end
		if sac_scale == "demanding_alpine_hiking" then way:Attribute("sac_scale", sac_scale) end
		if sac_scale == "difficult_alpine_hiking" then way:Attribute("sac_scale", sac_scale) end
	...
	-- Aerialways ('transportation' and 'transportation_name', plus 'transportation_name_detail')
	if aerialway~="" then
		way:Layer("transportation", false)
	...
	if building~="" then
		...
		SetBuildingAmenityAttributes(way)
		SetMinZoomByArea(way)
	end
...
function SetBuildingHeightAttributes(way)
	...
	-- Set if amenityplace_of_worship is present to a certain default value
    -- Set if amenityplace_of_worship is present to a certain default value
...
function SetBuildingAmenityAttributes(way)
	local amenity = (way:Find("amenity"))
